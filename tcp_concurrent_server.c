...
void fork_for_listen(int sock);
void fork_for_client(int sock, int connfd, struct sockaddr_in client);
...
void fork_for_listen(int sock)
{
  ...
  if ( (pid = fork() ) == 0) {
    ...
    while(1){
      ...
      fork_for_client(sock, connfd, client);
